...
df = pd.read_csv (r'/Users/jack/Documents/Cranfield/SDC/data_test_3_proccessed2.csv')
#print(df)
...
plt.figure('Speed')
plt.plot([x/4 for x in range(len(df['y_speed']))], df['y_speed'])
plt.plot([x/4 for x in range(len(df['y_speed']))], target)
plt.plot([x/4 for x in range(len(df['y_speed']))], lower_bound)
plt.plot([x/4 for x in range(len(df['y_speed']))], upper_bound)
...
plt.figure('DC')
plt.plot([x/4 for x in range(len(df['y_speed']))], df['Duty cycle'])
